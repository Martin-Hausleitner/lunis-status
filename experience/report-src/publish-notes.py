...
import json,re
proof=json.loads(Path('proof/report-v4/report-e2e.json').read_text())
if proof['status']!='PASSED' or proof['failed']!=0:raise RuntimeError('Do not publish unverified report notes')
root=Path('README.md');old=root.read_text()
start='<!-- LUNIS_REPORT_V4_START -->';end='<!-- LUNIS_REPORT_V4_END -->'
section=f'''{start}
...
Die Bilder oben wurden mit Chromium aus dem neuen Berichtsbuild aufgenommen. Der Browserlauf hat **{proof['passed']} Prüfgruppen bestanden**. Sie sind nicht mit den historischen Portal-Screenshots zu verwechseln. Die innerhalb des Berichts verwendeten Portalbilder stammen aus Cloud-Lauf `35720887432` und zeigen nur synthetische Testdaten.
...
{end}
'''
if start in old:old=re.sub(re.escape(start)+'.*?'+re.escape(end),section.strip(),old,flags=re.S)
else:old=section+'\n'+old
root.write_text(old)
Path('experience/bericht/REPORT.md').write_text('# Bericht V4 · Build-Abnahme\n\n'+section+'\nDie vollständige Cloud-Portal-Abnahme und der Test der veröffentlichten Berichtsversion laufen anschließend als eigene Workflow-Schritte. Maßgeblich ist deren tatsächliches Ergebnis.\n')
print('REPORT_README_UPDATED')
